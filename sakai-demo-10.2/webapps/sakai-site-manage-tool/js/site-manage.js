...
/*
 ...
 args:class of trigger, id of dialog, message strings
 */
sakai.getGroupInfo = function(trigger, dialogTarget, memberstr, printstr, tablestr1,tablestr2,tablestr3,roleIdNameMap){
    utils.startDialog(dialogTarget);
	$('.' + trigger).click(function(e){
        ...
        jQuery.getJSON(groupURL, function(data){
            $.each(data.membership_collection, function(i, item){
                list = list + "<tr><td>" + count + ")&nbsp;" + item.userSortName + "</td><td>" + roleIdNameMap[item.memberRole]+ "</td><td><a href=\'mailto:" + item.userEmail + "\'>" + item.userEmail + "</a></td></tr>";
                count = count + 1;
            });
...
sakai.siteTypeSetup = function(){
    ...
    if ($('#templateControls').val() !== '') {
     ...
     if($('#selectTermTemplate').selectedIndex === 0){
